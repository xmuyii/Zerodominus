...
try:
    from supabase_db import (
        ...
        add_gold, spend_gold, present_charges,
        claim_daily_login_credits, award_scoreboard_credits,claim_daily_teleports, start_operation, MARCH_DURATION_SECONDS, activate_energy_cell_from_backpack,
    ...
    try:
        while eng.running:
            try:
                ...
                # ─── 2. WEEKLY WINNERS BANNER (Updated to use Supabase) ────────────────────────
                winners_text = ""
                try:
                    from supabase_db import _current_week_key, supabase
                    _this_week = _current_week_key()
                    
                    if _winners_shown_week != _this_week:
                        # Fetch directly from the Supabase table managed by save_weekly_winners
                        res = supabase.table('last_week_winners').select('*').order('rank', desc=False).execute()
                        last_winners = res.data or []

                        # Fallback to All-Time Leaderboard if the table happens to be empty
                        if not last_winners:
                            try:
                                _lb = await asyncio.to_thread(get_alltime_leaderboard, limit=3)
                                last_winners = [{'username': p['username'], 'points': p['points']} for p in _lb if not p.get('is_bot', False)]
                            except Exception:
                                last_winners = []

                        if last_winners:
                            winners_text = "🏆 *LAST WEEK'S TOP PLAYERS* 🏆\n"
                            for i, p in enumerate(last_winners[:3]):  # Show top 3
                                medal = ["🥇", "🥈", "🥉"][i]
                    ...
                    print(f"[ERROR] Loading last week winners from Supabase: {e}")
...
async def callback_train_menu(callback: types.CallbackQuery):
    ...
    try:
        from training_system import UNITS, format_training_status
    except ImportError:
        await callback.answer("Training system unavailable", show_alert=True)
        return
    ...
    try:
        queue_display = format_training_status(u_id)
    except Exception:
        queue_display = ""

    # ── Build message ─────────────────────────────────────────────────────
    msg  = "🎖️ *MILITARY TRAINING CENTER*\n"
    msg += "━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    ...
    for unit_key, unit_data in UNITS.items():
        count = military.get(unit_key, 0)
        if count > 0:
            msg += f"  {unit_data['name']}: **{count}**\n"
            has_troops = True
    if not has_troops:
        msg += "  _No troops yet. Train some below._\n"

    # Active training queue
    if queue_display and "No active" not in queue_display and "queue" not in queue_display.lower():
    ...
    for unit_key, unit in UNITS.items():
        req_level  = unit.get("min_level", 1)
        req_research = unit.get("research_required")

        # Check locks
        level_ok    = level >= req_level
        research_ok = (not req_research) or researches.get(req_research, False)

        if level_ok and research_ok:
            current = military.get(unit_key, 0)
            # Build cost string
            costs = unit.get("costs", {})
            cost_parts = []
            for res in ("wood", "bronze", "iron"):
                if costs.get(res, 0):
                    cost_parts.append(f"{costs[res]}{res[0].upper()}")
            cost_str = "+".join(cost_parts) if cost_parts else "Free"
            time_s   = unit.get("train_time", 30)
            time_str = f"{time_s//60}m" if time_s >= 60 else f"{time_s}s"

            btn_label = f"{unit['name']} [{current}] — {cost_str} / {time_str}"
            rows.append([InlineKeyboardButton(
                text=btn_label,
                callback_data=f"train_{unit_key}"
            )])
        else:
            has_locked = True
            lock_reason = f"Lv{req_level}" if not level_ok else f"Research: {req_research}"
            locked_txt += f"  🔒 {unit['name']} — {lock_reason}\n"

    if has_locked:
        msg += locked_txt

    rows.append([InlineKeyboardButton(text="⬅️ Back to Base", callback_data="menu_base")])
...
from sector_dominance import (
    ...
    format_ruler_vision, declare_pretender, kb_phase_info
)
...
async def callback_build_confirm(callback: types.CallbackQuery):
    ...
    msg += f"Upgrading to Level {current_level + 1}\n"
    msg += f"Effect: {building['description']}\n\n"
    ...
    if prog:
        ...
        msg += f"⏱️ Build Time: {time_str}\n"
        msg += f"{format_build_progress_bar(prog)}\n\n"

    msg += "_Progress bar stays visible on your dashboard and in My Base._"

    keyboard = [
        [InlineKeyboardButton(text="🏗️ Build Another", callback_data="build_menu"),
...
async def _render_main_hud(message, user: dict, u_id: str, edit: bool = False):
    ...
    try:
        from power_system import calculate_player_power
    except ImportError:
        try:
            from power_system_v2 import get_total_power as calculate_player_power
        except ImportError:
    ...
    # Building queue
    try:
        bq = format_building_queue_display(user)
        if bq and bq.strip():
            progress_parts.append(bq.strip())
    except Exception:
        pass

    # Training queue
    try:
        from training_system import get_training_queue_display
        tq = get_training_queue_display(u_id)
        if tq and tq.strip():
            progress_parts.append(tq.strip())
    except Exception:
        try:
    ...
    # Research queue
    try:
        from research_tree import format_active_research
        rq = format_active_research(user)
        if rq and rq.strip():
            progress_parts.append(rq.strip())
    except Exception:
        pass
...
# ── 1. Define the base button matrix ──
    keyboard_rows = [
        [
    ...
    # ── 2. Conditionally append rows safely ──
    if present_charges > 0:
    ...
    # ── 3. Initialize the markup with the required inline_keyboard argument ──
    kb = InlineKeyboardMarkup(inline_keyboard=keyboard_rows)

    # ── 4. Dispatch text renders ──
    if edit:
        try:
            await message.edit_text(hud, parse_mode="HTML", reply_markup=kb)
        except Exception:
...
async def cb_menu_back_to_hud(callback: types.CallbackQuery):
    ...
    await _render_main_hud(callback.message, user, u_id, edit=True)
...
async def cb_menu_base(callback: types.CallbackQuery):
    """
    My Base — the complete base hub.
    Shows: resources, active progress bars for ALL activities,
    completed buildings as interactive inline buttons.
    """
    u_id = str(callback.from_user.id)
    user = get_user(u_id)
    from wiring_hooks import on_user_action
    ...
    # Complete any finished buildings/training first
    user = check_and_complete_buildings(user)
    save_user(u_id, user)
    ...
    # 1. Building queue
    bq_display = format_building_queue_display(user)
    if bq_display and bq_display.strip():
        msg += f"\n{bq_display}"
        any_progress = True
    ...
    try:
        from training_system import get_training_queue_display
        tq_display = get_training_queue_display(u_id)
        if tq_display and tq_display.strip():
            msg += f"\n{tq_display}"
            any_progress = True
    except Exception:
        try:
            from training_system import format_training_status
            ts = format_training_status(u_id)
            if ts and "No active" not in ts and ts.strip():
                msg += f"\n⚔️ *TRAINING:*\n{ts}\n"
                any_progress = True
        except Exception:
            pass
    ...
    try:
        from research_tree import format_active_research
        rq_display = format_active_research(user)
        if rq_display and rq_display.strip():
            msg += f"\n{rq_display}"
            any_progress = True
    except Exception:
        pass
    ...
    if not any_progress:
        msg += "\n_No active construction, training, or research._\n"

    # ── Army summary ──────────────────────────────────────────────────────
    total_troops = sum(v for v in military.values() if isinstance(v, int))
    if total_troops > 0:
        msg += f"\n⚔️ *ARMY:* {total_troops} troops\n"
        for unit, count in military.items():
            if count > 0:
                msg += f"  {unit.capitalize()}: **{count}**\n"

    msg += "\n━━━━━━━━━━━━━━━━━━━━━━━━━\n"

    # ── Keyboard — completed buildings as interactive buttons ──────────────
    rows = []
    ...
    if buildings:
        rows.append([InlineKeyboardButton(text="─── MY BUILDINGS ───", callback_data="noop")])
        building_row = []
        for building_id, level in sorted(buildings.items()):
            try:
                bname = BUILDING_TYPES.get(building_id, {}).get("name", building_id.replace("_", " ").title())
            except Exception:
                bname = building_id.replace("_", " ").title()
            btn = InlineKeyboardButton(
                text=f"{bname} Lv{level}",
                callback_data=f"building_detail_{building_id}"
            )
            building_row.append(btn)
            if len(building_row) == 2:
...
@dp.callback_query(lambda q: q.data.startswith("building_detail_"))
async def cb_building_detail(callback: types.CallbackQuery):
    """
    Show the interior of a specific completed building.
    Each building has unique actions available inside it.
    """
    u_id        = str(callback.from_user.id)
    user        = get_user(u_id)
    from wiring_hooks import on_user_action
    ...
    building_id = callback.data.replace("building_detail_", "")
    buildings   = user.get("buildings", {}) or {}
    ...
    level = buildings[building_id]

    try:
        bdata = BUILDING_TYPES.get(building_id, {})
    except Exception:
        bdata = {}

    bname = bdata.get("name", building_id.replace("_", " ").title())
    bdesc = bdata.get("description", "A base structure.")

    # ── Per-building content ───────────────────────────────────────────────
    BUILDING_CONTENT = {
        "training_grounds": {
            "desc":   "Increases troop training speed. The beating heart of your military.",
            "emoji":  "🏋️",
            "action_buttons": [
        ...
        "barracks": {
            "desc":   "Houses your troops. Higher level = more troop capacity.",
            "emoji":  "🏕️",
            "action_buttons": [
                [InlineKeyboardButton(text="🎖️ Train Troops", callback_data="train_menu"),
                 InlineKeyboardButton(text="⚔️ View Army",   callback_data="menu_battle")],
            ],
        },
        "armory": {
            "desc":   "Stores and upgrades weapons. Unlocks advanced troop types.",
            "emoji":  "⚔️",
            "action_buttons": [
                [InlineKeyboardButton(text="🗡️ Weapons",      callback_data="menu_weapons"),
                 InlineKeyboardButton(text="🎖️ Train Elite",  callback_data="train_menu")],
            ],
        },
        "war_room": {
            "desc":   "Strategic command center. Boosts attack power and march speed.",
            "emoji":  "🗺️",
            "action_buttons": [
                [InlineKeyboardButton(text="⚔️ Attack",        callback_data="menu_battle"),
                 InlineKeyboardButton(text="🔭 Scout",         callback_data="menu_scout")],
            ],
        },
        "storage": {
            "desc":   "Protects resources from raiders. Higher level = more protected.",
            "emoji":  "🏚️",
            "action_buttons": [
        ...
        "mine": {
            "desc":   "Passively generates iron and stone over time.",
            "emoji":  "⛏️",
            "action_buttons": [
                [InlineKeyboardButton(text="📦 View Resources", callback_data="menu_resources")],
            ],
        },
        "farm": {
        ...
        "trap_factory": {
            "desc":   "Manufactures traps. Higher level = more trap capacity.",
            "emoji":  "🔱",
            "action_buttons": [
                [InlineKeyboardButton(text="🔱 Manage Traps",  callback_data="show_trap_list")],
            ],
        },
        "walls": {
            "desc":   "Defensive perimeter. Reduces damage from raids.",
            "emoji":  "🏯",
            "action_buttons": [
                [InlineKeyboardButton(text="🛡️ Defense Status", callback_data="base_defense")],
            ],
        },
        "infirmary": {
            "desc":   "Recovers wounded troops after battle instead of losing them permanently.",
            "emoji":  "🏥",
            "action_buttons": [
                [InlineKeyboardButton(text="💊 Recover Troops", callback_data="menu_battle")],
            ],
        },
        "cemetery": {
            "desc":   "Fallen troops provide morale and XP bonuses. Honour your dead.",
            "emoji":  "⚰️",
            "action_buttons": [
                [InlineKeyboardButton(text="📊 View Stats",    callback_data="menu_profile")],
            ],
        },
        "base_hq": {
            "desc":   "Command headquarters. Level determines what can be built.",
            "emoji":  "🏰",
            "action_buttons": [
    ...
    content      = BUILDING_CONTENT.get(building_id, {})
    content_desc = content.get("desc", bdesc)
    content_emoji = content.get("emoji", "🏗️")
    action_btns  = content.get("action_buttons", [])
    ...
    msg  = f"{content_emoji} *{bname}* — Level {level}\n"
    msg += "━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    msg += f"_{content_desc}_\n\n"

    # Building-specific stats
    if building_id == "barracks" or building_id == "training_grounds":
        total = sum(v for v in military.values() if isinstance(v, int))
        msg += f"*Current Army:* {total} troops\n"
        for unit, count in military.items():
            if count > 0:
                msg += f"  {unit.capitalize()}: **{count}**\n"
        ...
        total_traps = sum(v for v in traps.values() if isinstance(v, int))
        msg += f"*Traps Deployed:* {total_traps}\n"
        for trap_id, count in traps.items():
            if count > 0:
                try:
                    tname = TRAP_TYPES.get(trap_id, {}).get("name", trap_id)
                except Exception:
                    tname = trap_id
                msg += f"  {tname}: {count}\n"
    ...
    # Upgrade info
    try:
        from build_system import calculate_building_cost
        next_cost = calculate_building_cost(building_id, level + 1)
        cost_str  = " + ".join(f"{v} {k}" for k, v in next_cost.items())
        msg += f"\n📈 *Upgrade to Lv{level+1}:* {cost_str}\n"
    except Exception:
        pass

    # Check if upgrading
    bq = user.get("building_queue", {}) or {}
    if building_id in bq:
        prog = get_building_progress(user, building_id)
        if prog:
            msg += f"\n⏳ *Upgrading:*\n{format_build_progress_bar(prog)}\n"

    msg += "\n━━━━━━━━━━━━━━━━━━━━━━━━━"

    # Keyboard: action buttons + upgrade + back
    rows = list(action_btns)
    rows.append([InlineKeyboardButton(
        text=f"⬆️ Upgrade to Lv{level+1}",
        callback_data=f"build_{building_id}"
    )])
    rows.append([InlineKeyboardButton(text="⬅️ Back to Base", callback_data="menu_base")])
...
# Ensure you pull the initialized client instance, NOT the raw module package
from supabase_db import supabase, get_user, save_user 
from wiring_hooks import on_user_action

# ─── 1. THE CALLBACK BUTTON HANDLER ───
@dp.callback_query(F.data.startswith("teleport_"))
async def cb_teleport(callback: types.CallbackQuery):
    """Handles teleportation when a player clicks a sector button."""
    try:
        await callback.answer()
    except Exception:
        pass

    # Extract the sector number from callback data (e.g., "teleport_5" -> 5)
    try:
        target_sector = int(callback.data.split("_")[1])
    except (IndexError, ValueError):
        await callback.message.answer("❌ Invalid sector selection.")
        return

    # Process the core teleportation logic using the shared helper below
    await execute_teleportation(
...
# ─── 2. THE TEXT COMMAND HANDLER ───
@dp.message(_cmd("teleport"))
async def cmd_teleport(message: types.Message):
    """Teleport commander via chat command (e.g., /teleport 3)."""
    if message.chat.type != "private":
        await message.answer("🃏 *GameMaster:* \"Teleport in *private*.\"", parse_mode="Markdown")
        return
    
    # Parse target sector from text input
    parts = message.text.strip().split()
    target_sector = None
    if len(parts) > 1:
        try:
...
# ─── 3. THE SHARED LOGIC ENGINE ───
async def execute_teleportation(message: types.Message, user_id: int, target_sector: int):
    """Shared function executing the DB mutations and encounter rolls."""
    u_id = str(user_id)
    user = get_user(u_id)
    on_user_action(u_id, supabase)
    ...
    # Update local dictionary cache to prevent save overwrites
    user['commander_location'] = target_sector
    save_user(u_id, user)

    # Force database synchronization using the imported database instance variable
    try:
        supabase.table("users").update({"commander_location": target_sector}).eq("user_id", u_id).execute()
    except Exception as e:
        logging.error(f"Failed to update commander_location in Supabase: {e}")

    # Render graphics/text frames
    detailed_info = format_sector_display(target_sector, divider)
    await message.answer(detailed_info, parse_mode="HTML")
    ...
    # 🎰 Check for procedural combat events
    player_level = user.get('level', 1)
    should_attack, reason = should_trigger_bandit_attack(player_level, target_sector)
    ...
    if should_attack:
        ...
        # Sync state down securely to eliminate race states
        user['commander_location'] = target_sector
        user['current_encounter'] = encounter
        save_user(u_id, user)
...
async def execute_teleport_logic(u_id: str, target_sector: int, trigger_event):
    """
    Handles internal data mapping, database persistence, and random encounter 
    generation for deployment into a tactical sector.
    """
    # 1. Load context and process user hooks
    user = get_user(u_id)
    if not user:
        if isinstance(trigger_event, types.Message):
            await trigger_event.answer("❌ Account not found.", parse_mode="Markdown")
        else:
            await trigger_event.answer("❌ Account not found.", show_alert=True)
        return
    ...
    if not sector:
        if isinstance(trigger_event, types.Message):
            await trigger_event.answer("❌ Sector not found.", parse_mode="Markdown")
        else:
            await trigger_event.answer("❌ Sector not found.", show_alert=True)
        return

    # 2. Persist location updates safely across systems
    user['commander_location'] = target_sector
    set_commander_location(u_id, target_sector)
    save_user(u_id, user)
    ...
    # 3. Determine execution channel (Message vs. CallbackQuery)
    # If it's a callback, we edit the button card inline instead of generating new spam
    if isinstance(trigger_event, types.CallbackQuery):
        all_sectors = load_sectors()
        info = all_sectors.get(target_sector, {})
        sname = info.get('name', f'Sector {target_sector}') if isinstance(info, dict) else str(info)
        
        with suppress(TelegramBadRequest):
            await trigger_event.answer("✨ Teleported!")
            await trigger_event.message.edit_text(
                f"✨ *TELEPORTED!*\n📍 *#{target_sector} {sname.upper()}*\n\nTeleport consumed.",
                parse_mode="Markdown"
            )
        # Re-assign message target context to handle following alerts downstream
        target_msg = trigger_event.message
    else:
        target_msg = trigger_event

    # 4. Display sector details and operational confirmation
    detailed_info = format_sector_display(target_sector, divider)
    await target_msg.answer(detailed_info, parse_mode="HTML")
    ...
    await target_msg.answer(confirmation, parse_mode="Markdown")
    teleport_charges = user.get('teleport_charges', 0)
    new_teleport_charge = teleport_charges - 1 if teleport_charges > 0 else 0
    user['teleport_charges'] = new_teleport_charge
    save_user(u_id, user)
    # 5. ═══════════════════════════════════════════════════════════════════════════
    #    CHECK FOR RANDOM BANDIT ENCOUNTER AT NEW SECTOR
    #    ═══════════════════════════════════════════════════════════════════════════
    player_level = user.get('level', 1)
    should_attack, reason = should_trigger_bandit_attack(player_level, target_sector)
    ...
    if should_attack:
        ...
        encounter_msg = format_bandit_encounter(encounter)
        await target_msg.answer(encounter_msg, parse_mode="Markdown")
...
@dp.message(_cmd("teleport"))
async def cmd_teleport(message: types.Message):
    """Teleport commander to a target sector via text input parameter."""
    if message.chat.type != "private":
        await message.answer("🃏 *GameMaster:* \"Teleport in *private*.\"", parse_mode="Markdown")
        return
    ...
    # Parse target sector out of arguments array
    parts = message.text.strip().split()
    target_sector = None
    if len(parts) > 1:
        try:
    ...
    await execute_teleport_logic(u_id, target_sector, message)
import re

@dp.callback_query(F.data.startswith("teleport_to_"))
async def cb_teleport_to(callback: types.CallbackQuery):
    """Processes button coordinates, consumes item inventory charge, and runs core deployment."""
    m = re.search(r'\d+', callback.data)
    if not m: 
        return
    ...
    if not (1 <= sector_id <= 9): 
        await callback.answer("That sector is locked!", show_alert=True)
        return
        
    # Burn/consume a single inventory item matching 'teleport' specifications
    for it in get_inventory_item(u_id):
        if it.get('type', '').lower() == 'teleport':
            remove_inventory_item(u_id, it.get('id'))
            break
            
    await execute_teleport_logic(u_id, sector_id, callback)


@dp.callback_query(F.data.startswith("teleport_"))
async def cb_general_teleport_menu(callback: types.CallbackQuery):
    """
    Catches parent menu initializers (e.g., 'teleport_') to dynamically direct 
    commanders toward random assignment drops if no exact sector id is appended.
    """
    u_id = str(callback.from_user.id)
    from wiring_hooks import on_user_action
    on_user_action(u_id, supabase)
    
    random_target = random.randint(1, 9)
    await execute_teleport_logic(u_id, random_target, callback)
...
async def cb_train_confirm(callback: types.CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            f"✅ *TRAINING QUEUED!*\n\n"
            f"⚔️ {unit['name']} × {quantity}\n"
            f"⏱️ Completes in: {time_str}\n\n"
            f"_Progress bar visible on your dashboard and in My Base._",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def weekly_reset_task(bot: Bot, chat_id: int):
    ...
    while True:
        try:
            ...
            # Target timezone: UTC+1 (WAT)
            now = datetime.now(timezone.utc) + timedelta(hours=1)  
            
            # Sunday = weekday 6. Check if within the 23:58-23:59 window just before Monday
            is_sunday_midnight = (
                now.weekday() == 6 and
                now.hour == 23 and
                now.minute >= 58
            ...
            # Use week ISO date string as a key to prevent duplicate triggers during the window
            ...
            if is_sunday_midnight and weekly_reset_done_for_week != current_week_id:
                ...
                print(f"[WEEKLY RESET] Starting automated leaderboard snapshot and reset at {now.isoformat()}")
                
                try:
                    # 1. Fetch the final standing rankings before resetting
                    lb_raw = get_weekly_leaderboard(limit=10)
                    # Filter out any lingering bot profiles
                    lb = [p for p in lb_raw if not p.get('is_bot', False)]
                    
                    # 2. Build the live chat announcement text block
                    announcement = (
                        f"{divider()}\n"
                    ...
                    if lb:
                        ...
                        # 3. Process rewards for the podium finishers (Top 3)
                        for i, p in enumerate(lb[:3]):
                            try:
                                reward_crates = 3 - i  # 1st place gets 3, 2nd gets 2, 3rd gets 1
                                ...
                                print(f"[WEEKLY RESET] Reward execution error for user {p.get('username')}: {re}")
                    ...
                    # Send announcement to the main group chat
                    try:
                        await bot.send_message(chat_id=chat_id, text=announcement, parse_mode="Markdown")
                    except Exception as msg_err:
                        print(f"[WEEKLY RESET] Failed to broadcast announcement message: {msg_err}")

                    # 4. DATABASE SNAPSHOT: Save winners directly to Supabase table
                    try:
                        from supabase_db import save_weekly_winners
                        save_weekly_winners()
                        print("[WEEKLY RESET] Successfully executed save_weekly_winners() in Supabase.")
                    except Exception as db_err:
                        print(f"[WEEKLY RESET] Failed to snapshot winners to Supabase table: {db_err}")

                    # 5. CLEAN UP FILE SYSTEM: Delete the conflicting local JSON file if it exists
                    try:
                        save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'last_week_winners.json')
                        if os.path.exists(save_path):
                            os.remove(save_path)
                            print(f"[WEEKLY RESET] Removed legacy cache file: {save_path}")
                    except Exception as file_err:
                        print(f"[WEEKLY RESET] Could not clear out local file layout: {file_err}")

                except Exception as inner_err:
                    print(f"[WEEKLY RESET] Main reset routine evaluation failed: {inner_err}")
                    
        except Exception as global_err:
            print(f"[WEEKLY RESET] Background worker encountered an operational loop fault: {global_err}")
